...
export const ConverterScreen = ({ category, onBack }) => {
    ...
    useEffect(() => {
        if (category === 'Currency') {
            fetch('https://v6.exchangerate-api.com/v6/a58c49b4447b407ffd227249/latest/USD')
                .then(res => res.json())
                .then(data => {
                    if (data && data.conversion_rates) setRates(data.conversion_rates);
    ...
    return (
        <div className="flex flex-col w-full bg-white dark:bg-dark-bg transition-colors">
            ...
            {/* Content */}
            <div className="px-5 py-6 bg-gray-50 dark:bg-dark-bg">
                {/* FROM BOX */}
                <div className="bg-white dark:bg-[#1A1A1A] p-5 rounded-2xl shadow-md border border-gray-100 dark:border-gray-800 flex flex-col gap-2 focus-within:ring-2 focus-within:ring-primary/50 transition-shadow">
                    <select value={fromUnit} onChange={(e) => setFromUnit(e.target.value)}
                        className="bg-transparent text-gray-500 dark:text-gray-400 font-medium text-lg outline-none cursor-pointer appearance-none hover:text-primary">
                        {unitNames.map(u => <option key={u} value={u} className="text-black">{u}</option>)}
                    </select>
                    <input type="text" placeholder="0" value={fromValue}
                        onFocus={() => setActiveInput('from')}
                        onChange={(e) => { setActiveInput('from'); setFromValue(e.target.value); }}
                ...
                <div className="flex justify-center my-4 relative z-10">
                    <motion.button whileTap={{ scale: 0.85, rotate: 180 }} onClick={handleSwap}
                        className="p-4 bg-primary text-white rounded-full shadow-lg hover:bg-opacity-90 transition-all focus:outline-none">
                        <ArrowDownUp size={24} />
                    </motion.button>
                </div>
                ...
                <div className="bg-white dark:bg-[#1A1A1A] p-5 rounded-2xl shadow-md border border-gray-100 dark:border-gray-800 flex flex-col gap-2 focus-within:ring-2 focus-within:ring-primary/50 transition-shadow">
                    ...
                    <input type="text" placeholder="0" value={toValue}
                        onFocus={() => setActiveInput('to')}
                        onChange={(e) => { setActiveInput('to'); setToValue(e.target.value); }}
